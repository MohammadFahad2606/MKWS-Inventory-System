...
export default function ProductDetailModal({ open, toggle, product, onEdit, onDelete, onTransaction }) {
  const navigate = useNavigate();
  if (!product) return null;

  // Calculate totals
  const totalIn = product.transactions?.filter((t) => t.type === "IN").reduce((sum, t) => sum + t.amount, 0) || 0;
  const totalOut = product.transactions?.filter((t) => t.type === "OUT").reduce((sum, t) => sum + t.amount, 0) || 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className="font-bold text-xl w-12 h-12 flex items-center justify-center rounded-full" style={{ background: "var(--color-primary)", color: "var(--color-text-on-primary)" }}>
          {product.name?.charAt(0) || "?"}
        </div>
        <div>
          <h2 className="font-semibold text-lg" style={{ color: "var(--color-text)" }}>{product.name}</h2>
          <p className="text-sm" style={{ color: "var(--color-muted)" }}>PID: {product.productId}</p>
          <p className="text-sm" style={{ color: "var(--color-muted)" }}>Buy Rate: {product.buyRate}</p>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-sm">
          <tbody>
            <tr className="font-semibold" style={{ background: "var(--color-primary)", color: "var(--color-text-on-primary)" }}>
            ...
            <tr style={{ background: "var(--color-highlight)", color: "var(--color-text)" }}>
      ...
      <div className="flex justify-between mt-6">
        ...
        <button
          ...
          className="px-4 py-2 rounded font-semibold"
          style={{ background: "var(--color-warning)", color: "var(--color-text-on-warning)" }}
        >
          EDIT
        </button>
        <button
          ...
          className="px-4 py-2 rounded font-semibold"
          style={{ background: "var(--color-success)", color: "var(--color-text-on-primary)" }}
        >
          TRANSACTIONS
        </button>
      </div>
